...
| **Cartridge** | ROM-only, MBC1 (ROM/RAM banking), MBC3 (with RTC latch/update), MBC5 (9-bit ROM bank, up to 15 RAM banks, rumble detection); battery-backed SRAM load/save to `.sav` |
| **Serial** | Blargg test ROM output piped to terminal |
| **Tests** | Passes Blargg `cpu_instrs` tests 01–02 |
| **GBC** | VRAM/WRAM banking, color palettes (BCPS/OCPD/OCPS/OBPD), HDMA, color scanline rendering, DMG compatibility palettes, double-speed mode (KEY1) |
...
│   ├── gb/                 # cpu.c, ppu.c, apu.c, timer.c, cartridge.c, gb.c
...
- [ ] Save states
- [X] Game Boy Advance support (implemented in cupidgba)
